...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            pos = tokenizer.pad_sequence(pos, pad_id=0, maxlen=85, dtype='int64', padding='post', truncating='post')
            deprel = [dep_vocab.stoi.get(t, dep_vocab.unk_index) for t in obj['deprel']]
            deprel = tokenizer.pad_sequence(deprel, pad_id=0, maxlen=85, dtype='int64', padding='post', truncating='post')
            mask = tokenizer.pad_sequence(obj['mask'], pad_id=0, maxlen=85, dtype='int64', padding='post', truncating='post')
            ...
            adj = np.pad(adj, (0, 85 - adj.shape[0]), 'constant')
            
            head = tokenizer.pad_sequence(obj['head'], pad_id=0, maxlen=85, dtype='int64', padding='post', truncating='post')
            length = obj['length']
